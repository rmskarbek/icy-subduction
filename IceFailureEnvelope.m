function [S_Compression, Tau_Compression, S_Tension, Tau_Tension, Sigma_Diff]...
    = IceFailureEnvelope(Depth, TempK, mu_flag, varargin)
%%% This function computes a failure envelope by assuming:
%%% 1. frictional failure takes place on optimally oriented planes.
%%% 2. Strain rates for ductile failure stress are determined kinematically from the slab
%%%    centerline curvature, following Buffett (2006).
...
%%% 10.29.2024 - Need to account for thin plate (plane strain?) stress state assumption:
%%% sigma_zz vanishes.
...
%%% Ductile differential stress from ice flow laws. Remove complex values that result from
%%% negative strain rates (i.e. tension).
